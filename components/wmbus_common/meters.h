...
struct MeterInfo
{
    std::string bus;  // The bus used to communicate with this meter. A device like /dev/ttyUSB0 or an alias like BUS1.
                 ...
                 // The bus can be the empty std::string, which means that it will fallback to the first defined bus.
...
// Parse a std::string like DEPRECATED,HIDE into bits.
...
private:
    ...
    function<std::string()> get_string_value_override_; // Callback to fetch the value from the meter.
    function<void(Unit,double)> set_numeric_value_override_; // Call back to set the value in the c++ object
    function<void(std::string)> set_string_value_override_; // Call back to set the value std::string in the c++ object

    // Lookup bits to std::strings.
